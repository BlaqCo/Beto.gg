...
async function restoreOpenPositionOnStartup() {
  ...
  const mine = (rows || []).find(r => r.league === "BTC60" && r.slug && r.slug.includes("-1h-"));
  if (!mine) return;
  const m = mine.slug.match(/(\d{4})-(\d{2})-(\d{2})-(\d{2})(\d{2})z$/);
  if (!m) {
    console.log(`  ⚠️ [BTC60] found a persisted open position ("${mine.slug}") but couldn't parse its window end time from the slug — leaving it untracked rather than guessing`);
    return;
  }
  const [, yyyy, mo, dd, hh, mi] = m;
  const windowStart = Date.UTC(+yyyy, +mo - 1, +dd, +hh, +mi);
  const endTime = new Date(windowStart + 60 * 60_000).toISOString();
  openPosition = {
  ...
  console.log(`  🔁 [BTC60] restored open position from persisted state after restart: "${mine.slug}" ${openPosition.side} @ ${(mine.entry*100).toFixed(0)}¢, endTime ${endTime}`);
}
...
async function checkNaturalResolution() {
  ...
  if (resolution) {
    ...
    try {
      ...
      console.log(`  ❌ [BTC60] Couldn't fetch closed markets for ${openPosition.slug}: ${err.message} — will retry next scan`);
      return;
    }
