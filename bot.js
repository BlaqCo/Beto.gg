 ...
 *
 * SHARP SHOOTER MODE (FADE THE EXTREME):
 *   - Activated by env var SHARP_SHOOTER=true
 *   - 10 concurrent slots, $5 flat bets, up to 3 entries per scan
 *   - Only enters at 25%+ confidence AND |bias| > 0.10
 *   - Smart side selection: reads question direction, matches signal
 *   - $50 daily loss limit: stops entering if session P&L < -$50
 *
 * NORMAL MODE:
...
const NORMAL_MAX          = 3;
const SS_MAX              = 10;
const SS_MIN_CONFIDENCE   = 0.25;   // skip weak signals
const SS_MAX_DAILY_LOSS   = 50;     // pause entries if down $50
const SS_BET_SIZE         = 5.00;   // $5 flat per bet
const SS_ENTRIES_PER_SCAN = 3;      // max 3 entries per scan cycle
...
export async function runScanCycle() {
  ...
  // ── New entries ──
  // Shuffle so all 6 coins get a fair shot — without this BTC always fills first
  ...
  for (const market of scalpMarkets) {
    ...
    // Prevent YES+NO on same question
    if (SS_MODE) {
    ...
    if (SS_MODE) {
      // Gate 1: confidence and bias minimums
      ...
      // Smart side: read question direction, match to signal
      const q       = (market.question || "").toLowerCase();
      const isBullQ = q.includes("rise above") || q.includes("reach") ||
                      q.includes("hit")         || q.includes("above") ||
                      q.includes("be above");
    ...
    try {
      ...
      recordBet({
        ...
        reasoning:          decision.reasoning || `${SS_MODE ? "⚡SS" : signals.activeStrategy} | bias:${signals.bias.toFixed(3)} | conf:${(signals.confidence*100).toFixed(0)}% | ${decision.side} @ ${(entryPrice*100).toFixed(0)}¢`,
