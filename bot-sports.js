...
let ENDGAME_MIN    = 0.55;   // lowered again — combined with band widening to get real volume flowing now
...
let NEAR_LOW_TOL  = 0.08;    // widened further — direct request to get bets flowing now, not another incremental nudge
...
async function _runScanCycleInner() {
  ...
  // Top-of-function scope, deliberately — used by the ranking sort, the
  // entry loop's model-edge relaxation, AND the funnel publish, which are
  // three separate, non-nested blocks later in this function. Anything
  // declared inside any ONE of those blocks is invisible to the other two;
  // only a declaration here, before all of them, is visible to all three.
  ...
  const BBO_BATCH_SIZE = 8;
  const BBO_BATCH_GAP_MS = 450;
  ...
  // it by grepping raw logs again next time.
  console.log(`  📶 BBO: ${bbosWithData.length}/${candidatePool.length} priced${rateLimitedCount ? `, ${rateLimitedCount} rate-limited (429)` : ", 0 rate-limited"}`);
